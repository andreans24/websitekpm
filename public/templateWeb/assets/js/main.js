...
(function() {
  ...
  function toggleScrolled() {
    ...
    if (!selectHeader || (!selectHeader.classList.contains('scroll-up-sticky') && 
      ...
      return; // Tidak ada header atau tidak sesuai kriteria
    }
    window.scrollY > 100 ? selectBody.classList.add('scrolled') : selectBody.classList.remove('scrolled');
  }
   ...
   */
  const mobileNavToggleBtn = document.querySelector('.mobile-nav-toggle');
  if (mobileNavToggleBtn) { // Pastikan tombol ada sebelum menambahkan event listener
  ...
  if (navmenuLinks.length) { // Pastikan ada link
  ...
  if (navmenuDropdowns.length) { // Pastikan dropdown ada
  ...
  if (scrollTop) { // Pastikan tombol ada
    function toggleScrollTop() {
      window.scrollY > 100 ? scrollTop.classList.add('active') : scrollTop.classList.remove('active');
    }
    scrollTop.addEventListener('click', (e) => {
   ...
   */
  function aosInit() {
    if (typeof AOS !== 'undefined') { // Pastikan AOS ada sebelum inisialisasi
      AOS.init({
        duration: 600,
        easing: 'ease-in-out',
        once: true,
  ...
  function initSwiper() {
    ...
    if (swiperElements.length) { // Pastikan elemen swiper ada
  ...
  if (faqItems.length) { // Pastikan elemen ada
   ...
   */
  let navmenulinks = document.querySelectorAll('.navmenu a');
  if (navmenulinks.length > 0) { // ✅ Perbaikan: pastikan elemen ada
    function navmenuScrollspy() {
      navmenulinks.forEach(navmenulink => {
        if (!navmenulink.hash) return;
        let section = document.querySelector(navmenulink.hash);
